...
if exists(select o.name from sysobjects o where o.name='serf_wra_mv_prep_hv_v2')
begin 
drop view serf_wra_mv_prep_hv_v2
end 
go 

create view serf_wra_mv_prep_hv_v2 
as 
select
sr.uid,
tlpin,
...
update_time,
updateed_by
from all_serf_hv_v2_wra sr

----
left join 
----
(select b.uid,b.srdate,max(b._submission_time) _submission_time
from (select sr1.uid,sr1.srdate,_submission_time from [mindr-live].dbo.all_serf_hv_v2_wra sr1
left join (select sr2.uid, sr2.srdate 
from [mindr-live].dbo.all_serf_hv_v2_wra sr2
