...
df <- read_dta("C:/Users/bdyer/OneDrive - Johns Hopkins/CHN/JiVitA/MiNDR/data/datasets/20241130_20241212/stata/PW_study/pregtrak.dta")
...
wtrak_pw_prepped <- wtrak_pw_p %>% 
  ...
  mutate(mpf_temp_out=case_when(mpf_not %in% c("Not met (up to 35 weeks GA)","No supplementation before MPF","MPF done - MPF Specimen - Not met ","Refused MPF","Under follow-up") ~as.character(mpf_not),
         ...
         lpf_not=case_when(
           ...
           is.na(mpf_met) & mpfb_status!=66 & lpf_status ==2 ~"Not met (up to any outcome)",
...
wtrak_pw_prepped %>% count(arm_pw,mpf_met)
...
wtrak_pw_prepped %>% count(mid_preg_status,late_preg_status,outc)

wtrak_pw_prepped_t <-wtrak_pw_prepped %>% 
  filter(is.na(mid_preg_status) 
         & is.na(late_preg_status) 
         & !is.na(outc) 
         # & saf_flow_status==1
         & lpf_status==1 & lpfb_status==1 )


wtrak_pw_prepped_t2 <-wtrak_pw_prepped %>% 
  filter(!is.na(lpf_met))

wtrak_pw_prepped_t2 %>% count(arm_pw,mpf_status,lpf_status,lpfb_status,lpf_met,outc)

!is.na(lpf_met)

wtrak_pw_prepped_t %>% count(outc)
...
wtrak_pw_prepped_lpf_nm_thruoutc <- wtrak_pw_prepped %>% 
  filter(lpf_not=="Not met (up to any outcome)") %>% 
  arrange(arm_pw,uid)
...
wtrak_pw_prepped_x_lpf_staty <- wtrak_pw_prepped %>% 
  filter(arm_pw=="W" & lpf_status==2)
