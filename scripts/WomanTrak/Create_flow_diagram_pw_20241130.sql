...
,current_ga = datediff(week,pw.uvf_lmp_date,'2024-11-30')
from wtrak_pw_prep pw 
left join 
--saf_first_supped 
...
and saf_flow_status = 1
and convert(smalldatetime,'2024-11-30',121) < s.schedule_gen_date -- this form is pending (current date < schedule generation date)
and datediff(week,coalesce(w.uvf_lmp_date,w.psf_lmp_date),convert(smalldatetime,'2024-11-30',121)) < 40 -- ga date is < 40 (max week that an mpf has had)
...
from wtrak_pw_prep pw 
--left join pw_log1 pl on pw.uid =pl.uid 
where lpf_schedule_date >= convert(smalldatetime,'2024-11-30',121)
