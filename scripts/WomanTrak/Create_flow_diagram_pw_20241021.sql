...
,current_ga = datediff(week,pw.uvf_lmp_date,'2024-10-15')
from wtrak_pw_prep pw 
left join 
--saf_first_supped 
...
update wtrak_pw_prep 
set mid_preg_status = case when mpf_status >1 and mid_preg_status is not null then mpf_status 
when mpfb_status >1  and mid_preg_status is not null then mpfb_status else mid_preg_status 
end 
from wtrak_pw_prep pw 
...
and saf_flow_status = 1
and convert(smalldatetime,'2024-10-15',121) < s.schedule_gen_date -- this form is pending (current date < schedule generation date)
and datediff(week,coalesce(w.uvf_lmp_date,w.psf_lmp_date),convert(smalldatetime,'2024-10-15',121)) < 40 -- ga date is < 40 (max week that an mpf has had)
...
from wtrak_pw_prep pw 
--left join pw_log1 pl on pw.uid =pl.uid 
where lpf_schedule_date >= convert(smalldatetime,'2024-10-15',121)
...
print 'mid pregnancy visit status'
select allocated_arm,saf_flow_status, mpf_status,mpf_status,mpfb_status,w.bgoutc ,mid_preg_status,count(*) from wtrak_pw w
where pef_consent =1 
and saf_flow_status = 1
--and allocated_arm ='B'
group by allocated_arm,saf_flow_status,mpf_status,mpfb_status,w.bgoutc,mid_preg_status
with rollup 
