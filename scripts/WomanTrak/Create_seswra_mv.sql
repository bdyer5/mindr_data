...
if exists(select o.name from sysobjects o where o.name='seswra_mv')
begin 
drop view seswra_mv 
end 
go 

create view seswra_mv 
as 
select
se.uid,
tlpin,
...
[end],
scheduleid
from     [mindr-live].dbo.all_ses_wra se inner join
shapla.dbo.jivitaweek as jw on jw.romandate = convert(smalldatetime, se.ssdate, 121)
left join 
(select b.uid,b.ssdate,max(b._submission_time) _submission_time
from (select se1.uid,se1.ssdate,_submission_time from [mindr-live].dbo.all_ses_wra se1
left join (select se2.uid, max(ssdate) ssdate 
from [mindr-live].dbo.all_ses_wra se2
